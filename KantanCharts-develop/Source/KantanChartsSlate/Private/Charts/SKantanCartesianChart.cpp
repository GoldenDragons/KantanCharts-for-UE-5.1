...
int32 SKantanCartesianChart::DrawAxes(const FGeometry& PlotSpaceGeometry, const FSlateRect& ClipRect, FSlateWindowElementList& OutDrawElements, int32 AxisLayerId, int32 LabelLayerId, FPlotMarkerData const& MarkerData) const
{
	...
	{
		float x = CartesianRangeMin.X;
		CartesianRangeMin.X = CartesianRangeMax.Y;
		CartesianRangeMax.X = x;
		//Swap<float>(CartesianRangeMin.X, CartesianRangeMax.X);
	}
	if (CartesianRangeMin.Y > CartesianRangeMax.Y)
	{
		float y = CartesianRangeMin.Y;
		CartesianRangeMin.Y = CartesianRangeMax.Y;
		CartesianRangeMax.Y = y;
		//Swap<float>(CartesianRangeMin.Y, CartesianRangeMax.Y);
	}
